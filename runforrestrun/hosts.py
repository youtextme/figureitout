...
def detect(project_root: Path | None = None) -> list[Host]:
    ...
    for host in CATALOG:
        ...
        if host.id == "openclaw":
            hit = True  # project .agents/skills + workspace paths; cheap to write
...
def _cursor_rule_paths(home: Path, root: Path) -> list[Path]:
    return [
        root / ".cursor" / "rules" / "run-forrest-run.mdc",
        home / ".cursor" / "rules" / "run-forrest-run.mdc",
        home / ".config" / "Cursor" / "rules" / "run-forrest-run.mdc",
        home / "Library" / "Application Support" / "Cursor" / "rules" / "run-forrest-run.mdc",
    ]
...
def extra_instruction_files(host: Host, project_root: Path) -> dict[str, str]:
    ...
    home = _home()
    files: dict[str, str] = {}
    if host.id in {"codex", "aider", "goose", "amp", "gemini", "cli-python", "agents-spec", "openclaw"}:
        files[str(root / "AGENTS.md")] = "agents"
    if host.id == "cursor":
        for path in _cursor_rule_paths(home, root):
            files[str(path)] = "rule"
    if host.id == "devin":
        ...
        files[str(home / ".config" / "devin" / "global_rules.md")] = "devin_global"
    ...
    if host.id == "openclaw":
        ...
        files[str(openclaw / "openclaw.json")] = "openclaw_config"
    return files
